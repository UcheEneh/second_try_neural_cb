...
def tokenizer(sentence):
    # str() in Python 3 is the equivalent to unicode() in Python 2 and can be used without problems.
    doc = nlp(str(sentence, errors='ignore'))
    ...
    if not gfile.Exists(vocabulary_path):
        ...
        with gfile.GFile(data_path, mode="r") as f:
            for line in tqdm(f):
                ...
                for token in tokens:
                    try:
                        vocab[token] += 1   # if the token/word in vocab, increment count by 1
                    except:
                        vocab[token] = 1    # if the token/word not in vocab, put it and initialize count to 1

            # Sort vocab in descending order (i.e least frequent first)
            ...
            # Use only first 150000 words
            if len(vocab_list) > max_vocabulary_size:
                vocab_list = vocab_list[:max_vocabulary_size]

            # Contains only words, not count
...
def initialize_vocabulary(vocabulary_path):
    """Initialize vocabulary from file.
        ...
        also return the transposed (reversed)-vocabulary ["dog", "cat"].
    ...
    if gfile.Exists(vocabulary_path):
        ...
        # Note: strip removes unnecessary whitespace at beginning of sentence
        rev_vocab = [line.strip() for line in rev_vocab]
        # Create a dict of token : weight (something like tfid))
        vocab = dict([(x, y) for (y, x) in enumerate(rev_vocab)])   # Should start=1 ???????
...
def sentence_to_token(sentence, vocabulary):
    ...
    return [vocabulary.get(w, UNK_ID) for w in words]   # if w not in words, return 3 (i.e UNK_ID)
