...
nlp = spacy.load('en')
genism_model = gensim.models.Word2Vec.load_word2vec_format(params.embeddings_dir, binary=True)

def embeddings_matrix():
    return genism_model.syn0
...
def specials_to_id(token):
    # return the id of special cases
    # return index for UNK for words not in vocab
    base = 3000000
    if token in special_map:
        # Return index of the special character: e.g: _GO = 0, _PAD = 1 ...
...
def sentence_to_ids(sentence):
    ...
    sentence = str(sentence, errors='ignore')
    for token in tokenizer(sentence):
        try:
            # using the word2vec model from genism
            vocab = genism_model.vocab[token]
            sent_ids.append(vocab.index)
        # if word not in genism_model vocab:
        except KeyError:
...
def ids_to_sentence(ids):
    ...
    for uid in ids:
        try:
            word = genism_model.index2word[uid]
            words.append(word)
        except IndexError:
